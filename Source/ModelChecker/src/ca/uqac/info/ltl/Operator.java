...
/**
 * Implementation of Boolean and LTL connectives.
 ...
 * quantifiers</strong>:
 * <ol>
 * <li>&forall; <i>x</i> &in; &pi; : &phi; means "for every <i>x</i> in &pi;,
 * &phi; holds". Here, <i>x</i> is a variable, and &pi; is an XPath expression
 * function used to fetch possible values for <i>x</i> inside the current
 * event. Hence, if events are XML messages, &pi; can be an
 * XPath expression retrieving values for <i>x</i>; for example, if &phi; is a
 * formula where <i>x</i> appears, then &forall; <i>x</i> &in <tt>/tag1/tag2</tt>
 * : &phi; means: every value at the end of <tt>/tag1/tag2</tt> satisfies
 * &phi;.
 * <li>&exist; <i>x</i> &in; &pi; : &phi; means "some <i>x</i> in &pi;". Hence,
 * &exist; <i>x</i> &in; <tt>/tag1/tag2</tt> : &phi; means: there exists some
 * value at the end of <tt>/tag1/tag2</tt> which satisfies &phi;.
 * </ol>
 * Note that the colon in the last two expressions <em>must</em> be surrounded
 * by spaces.
 * <p>
...
public abstract class Operator
{
  ...
  protected static Operator parseFromString(String s, Set<Atom> bound_variables) throws ParseException
  {
    ...
    if (isQuantifierStart(c))
    {
      ...
      oq.setPath(p);
      // Process operand
      s = s.substring(end_quantif + 3).trim();
    ...
    else
    {
      ...
      {	
        ...
        else
        {
          if (s.startsWith("\""))
          {
            // If the string is delimited with quotes, remove the quotes
